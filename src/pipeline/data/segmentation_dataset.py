...
from src.utils.data_subdirectories import (get_relative_image_dir, get_relative_mask_dir,
                                           get_relative_manual_image_dir, get_relative_manual_mask_dir)
...
class ManualSegmentationDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        self.root_dir = root_dir
        self.transform = transform
        self.image_dir = get_relative_manual_image_dir(root_dir)
        self.mask_dir = get_relative_manual_mask_dir(root_dir)
        # List only .jpg files in the image_dir
        self.files = [file for file in os.listdir(self.image_dir) if file.endswith('.jpg')]
    ...
    def __getitem__(self, idx):
        filename = self.files[idx]
        img_path = os.path.join(self.image_dir, filename)
        # Construct mask filename: replace '.jpg' with '_mask.png'
